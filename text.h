#ifndef SPOTIFY_CLONE_TEXT_H
#define SPOTIFY_CLONE_TEXT_H
#include "raylib.h"
    ...
    for (int i = 0, k = 0; i < length; i++, k++)
    {
        ...
        else
        {
            if (codepoint == '\n')
            {
                if (!wordWrap)
                {
                    textOffsetY += float(font.baseSize + font.baseSize/2)*scaleFactor + newLineOffsetY;
                    textOffsetX = 0;
                }
            }
            else
            {
                ...
                if ((textOffsetY + (float)font.baseSize*scaleFactor + newLineOffsetY) > rec.height) break;
            ...
            {
                textOffsetY += float(font.baseSize + (float)font.baseSize/2)*scaleFactor + newLineOffsetY;
                textOffsetX = 0;
                startLine = endLine;
                endLine = -1;
                glyphWidth = 0;
